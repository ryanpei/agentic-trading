...
from tests.conftest import adk_mock_riskguard_generator
...
async def test_execute_success_approved(
    riskguard_message_factory, mock_runner_factory, event_queue, adk_mock_riskguard_generator
):
...
@pytest.mark.asyncio
async def test_execute_missing_trade_proposal(mock_runner_factory, event_queue, adk_mock_riskguard_generator):
    mock_runner_instance = mock_runner_factory("riskguard.agent_executor")
    ...
    assert len(enqueued_message.parts) == 1
    assert isinstance(enqueued_message.parts[0].root, DataPart)
    assert "An internal error occurred: Missing 'trade_proposal' or 'portfolio_state' in data payload" in enqueued_message.parts[0].root.data["reason"]
...
async def test_execute_adk_runner_exception(
    ...
    assert len(enqueued_message.parts) == 1
    assert isinstance(enqueued_message.parts[0].root, DataPart)
    assert "An internal error occurred: ADK Borked" in enqueued_message.parts[0].root.data["reason"]
